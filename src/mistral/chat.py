...
from typing import Any, Dict, Literal, TypeAlias, TypeVar, overload
...
from mistralai import ChatCompletionResponse, Messages, Mistral, RetryConfig
from mistralai.extra.exceptions import MistralClientException
from pydantic import BaseModel

from mistral.serializer import MistralMessageSerializer
...
class ChatMistral(BaseChatModel):
    ...
    reasoning_models: list[str] | None = field(
        default_factory=lambda: ["magistral-medium-latest", "magistral-small-latest"]
    )
    ...
    async def ainvoke(
        ...
        mistral_messages: list[Messages] = MistralMessageSerializer.serialize_messages(messages)
        ...
        try:
            if output_format is None:
                response = await client.chat.complete_async(model=self.model, messages=mistral_messages, **model_params)
                usage = self._get_usage(response)
                return ChatInvokeCompletion(completion=response.choices[0].message.content or "", usage=usage)
            else:
                ...
                response = await client.chat.complete_async(
                    model=self.model, messages=mistral_messages, tools=tools, tool_choice="required", **model_params
                )
